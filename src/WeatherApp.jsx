import { useState } from 'react';
import './WeatherApp.css'

export const WeatherApp = () => {

    const [city, setCity] = useState('')
    const [weatherData, setWeatherData] = useState(null)

    const urlBase = `https://api.openweathermap.org/data/2.5/weather`;
    let API_KEY = "INSERT_API_KEY";
    const diffKelvin = 273.15; // Se usa para conversion a grados Celcius

    const fetchWeatherData = async() => {

        try {
            const response= await fetch(`${urlBase}?q=${city}&appid=${API_KEY}&lang=es`)
            const data = await response.json()
            setWeatherData(data)
            console.log(data)
        } catch (error) {
            console.error('Ocurrio un error gravisimo: ',error)
        }
    }

    const handleSubmit = (event) => {
        event.preventDefault();
        fetchWeatherData()
        console.log(city)

    }
    const handleCityChange = (event) => {
        setCity(event.target.value)
    }

    return (
        <>
            <div className='container'>
                <h1>Esta es la aplicacion del Clima</h1>
                <form onSubmit={handleSubmit}>
                    <input
                        type="text"
                        placeholder='Ingresa una ciudad'
                        value={city}
                        onChange={handleCityChange}
                    />
                    <button type="submit">Buscar</button>
                </form>

                {weatherData && (

                    <div>
                        <h2>
                            {weatherData.name},{weatherData.sys.country}
                        </h2>
                        <p>La temperatura actual es: {Math.floor(weatherData.main.temp -diffKelvin)}°C</p>
                        <p>La sensacion termica actual es: {Math.floor(weatherData.main.feels_like -diffKelvin)}°C</p>
                        <p>La condicion meteorologica actual: {weatherData.weather[0].description}</p>
                        <img src={`https://openweathermap.org/img/wn/${weatherData.weather[0].icon}@2x.png`} alt={weatherData.weather[0].description} />
                    </div>
                )}
            </div>
        </>
    )
}
